/* ===== Original inline script 1 from pricing.html ===== */
(function() {
...
const enrollFormView=document.getElementById('enrollFormView');
const successView=document.getElementById('successView');
const selectedProgramName=document.getElementById('selectedProgramName');
const selectedProgramFee=document.getElementById('selectedProgramFee');
const successProgramName=document.getElementById('successProgramName');
const successProgramDetail=document.getElementById('successProgramDetail');

function openProgramModal(name,fee){
  selectedProgramName.textContent=name;
  selectedProgramFee.textContent=fee;
  successProgramName.textContent=name;
  successProgramDetail.textContent=name;
  enrollFormView.style.display='block';
...
document.querySelectorAll('.program-choice').forEach(btn=>{
  btn.addEventListener('click',e=>{
    e.preventDefault();
    openProgramModal(btn.dataset.program,btn.dataset.fee);
  });
});
modalClose.addEventListener('click',closeProgramModal);
modalBackdrop.addEventListener('click',closeProgramModal);
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&programModal.classList.contains('open'))closeProgramModal()});
programForm.addEventListener('submit',e=>{
  e.preventDefault();
  enrollFormView.style.display='none';
  successView.classList.add('show');
});
document.getElementById('successClose').addEventListener('click',closeProgramModal);
...
/* ===== Original inline script 2 from pricing.html ===== */
(function() {
(function(){
...
/* ===== Original inline script 3 from pricing.html ===== */
(function() {
(function(){
...
/* ===== Original inline script 4 from pricing.html ===== */
(function() {
