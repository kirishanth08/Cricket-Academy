/* ===== Original inline script 1 from attendance.html ===== */
(function() {
...
function applyFilter(label){
  const abbr=MONTHS[label.split(' ')[0]]||label.slice(0,3);
  let present=0,absent=0,total=0;
  rows.forEach(function(r){
    const date=r.cells[0].textContent.trim();
    const show=date.startsWith(abbr);
    r.style.display=show?'':'none';
    if(show){
      total++;
      const st=r.cells[2].textContent.trim();
      if(st==='Present')present++;else if(st==='Absent')absent++;
    }
  });
  const boxes=document.querySelectorAll('.stat-box strong');
  if(boxes.length>=3){boxes[0].textContent=present;boxes[1].textContent=absent;boxes[2].textContent=total;}
  const donut=document.querySelector('.donut span');
  if(donut&&total)donut.textContent=Math.round(present/total*100)+'%';
  const head=document.querySelector('.card-head strong');
  if(head)head.textContent='Attendance · '+label;
}
document.getElementById('monthSelect').onchange=e=>applyFilter(e.target.value);
applyFilter(document.getElementById('monthSelect').value);
})();

/* ===== Original inline script 2 from attendance.html ===== */
(function() {
...
/* ===== Original inline script 3 from attendance.html ===== */
(function() {
(function(){
...
/* ===== Original inline script 4 from attendance.html ===== */
(function() {
(function(){
