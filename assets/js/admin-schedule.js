/* ===== Original inline script 1 from admin-schedule.html ===== */
(function() {
...
const sm=document.getElementById('sessionModal');document.getElementById('addSession').onclick=()=>sm.classList.add('show');document.getElementById('sessionClose').onclick=()=>sm.classList.remove('show');document.getElementById('sessionCancel').onclick=()=>sm.classList.remove('show');
document.getElementById('sessionForm').onsubmit=e=>{e.preventDefault();sm.classList.remove('show');notify('Training session saved')};
document.querySelectorAll('.reschedule').forEach(b=>b.onclick=()=>{sm.classList.add('show');notify('Edit the session details and save the new schedule')});
document.getElementById('scheduleFilter').onchange=e=>document.querySelectorAll('.event').forEach(x=>x.style.display=e.target.value==='all'||x.dataset.program===e.target.value?'block':'none');
})();
